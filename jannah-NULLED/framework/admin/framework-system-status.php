...
if( ! class_exists( 'TIE_SYSTEM_STATUS' )){
	...
	class TIE_SYSTEM_STATUS{
		...
		private function _print_theme_info(){
			...
			<table class="tie-status-table status-report widefat" cellspacing="0">
				...
				<tbody>
					...
					<?php if( $demo = get_option( 'tie_jannah_installed_demo' ) ): ?>
						<tr>
							<td data-export-label="Demo Installed"><?php esc_html_e( 'Latest Demo Installed', 'jannah' ); ?>:</td>
							<td><?php echo $demo; ?></td>
						</tr>
					<?php endif ?>

					<?php if( $switcher = get_option( 'switch_to_jannah' ) ):
									  $switcher = explode( '.', $switcher );
										if( is_array( $switcher ) && ! empty( $switcher[0] ) ){ ?>
						<tr>
							<td data-export-label="Switched From"><?php esc_html_e( 'Switched From', 'jannah' ); ?>:</td>
		...
		private function _print_report(){ ?>
			...
			jannah_theme_option(
				array(
					'text'  => sprintf( esc_html__( 'Looking for a good web hosting company? %1$sCheck our recommendations!%2$s', 'jannah' ), '<a target="_blank" href="https://tielabs.com/wordpress-hosting/"><strong>', '</strong></a>' ),
					'type'  => 'message',
				));
