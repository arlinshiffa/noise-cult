...
define( 'PT_OCDI_URL', get_template_directory_uri().'/framework/admin/inc/one-click-demo-import/' );
define( 'PT_OCDI_PATH', get_template_directory().'/framework/admin/inc/one-click-demo-import/' );
...
class PT_One_Click_Demo_Import {
	...
	public function create_plugin_page(){
		$plugin_page_setup = apply_filters( 'pt-ocdi/plugin_page_setup', array(
				...
				'page_title'  => esc_html__( 'Choose the demo which you want to import', 'jannah' ),
				'menu_title'  => esc_html__( 'Demo Import', 'jannah' ),
				'capability'  => 'import',
	...
	// by TieLabs ***************************************************************
		...
		if( isset( $_REQUEST['uninstall-demo'] ) ){
			...
			if( 'yes' == $_REQUEST['uninstall-demo'] && check_admin_referer( 'uninstall_demos', 'uninstall_demos_nonce' ) ){
				JANNAH_DEMO_IMPORTER::_uninstall();
			}
			...
			elseif( 'done' == $_REQUEST['uninstall-demo'] ){
				...
				<div style="display: block;" class="ocdi__response js-tie-ajax-response js-ocdi-ajax-response">
					...
					<h3 class="is-done"><?php esc_html_e( 'Demo data has been successfully uninstalled', 'jannah' ); ?></h3>
				</div>
				<?php
			}
			return;
	...
	update_option( JANNAH_THEME_FOLDER .'_demos_count', $demos_count );

	?>

	<div id="tie-import-wrap" class="ocdi wrap about-wrap tie-about-wrap">

		<?php TIE_WELCOME_PAGE::_head_section( 'demos' ); ?>
		...
		$plugin_page_setup = apply_filters( 'pt-ocdi/plugin_page_setup', array(
			...
			if( ! get_option( 'tie_token_'.JANNAH_THEME_ENVATO_ID ) ){
				...
				if( is_rtl() ){
					...
					jannah_theme_option(
						array(
							'text'  => esc_html__( 'All Demos Support RTL.', 'jannah' ),
							'type'  => 'message',
						));
				}
				...
				if( get_option( JANNAH_THEME_FOLDER .'_history' ) ){
					...
					$uninstall_msg = '
						<strong>'. esc_html__( 'Tip:', 'jannah' ) .'</strong> '.
						esc_html__( 'You need to uninstall the imported demo before importing another one.', 'jannah' ) . '
						<a id="tie-uninstall-demo" class="tie-primary-button button button-primary button-large tie-button-red" href="'. esc_url( $uninstall_url ) .'">'. esc_html__( 'Uninstall the Demo Data', 'jannah' ) .'</a>';
		...
		<div class="ocdi__intro-text">
			<p>
				<?php esc_html_e( 'Importing demo data (post, pages, images, theme settings, ...) is the easiest way to setup your theme. It will allow you to quickly edit everything instead of creating content from scratch. When you import the data, the following things might happen:', 'jannah' ); ?>
			</p>

			<ul>
				<li><?php esc_html_e( 'No existing posts, pages, categories, images, custom post types or any other data will be deleted or modified.', 'jannah' ); ?></li>
				<li><?php esc_html_e( 'Posts, pages, images, widgets and menus will get imported.', 'jannah' ); ?></li>
				<li><?php esc_html_e( 'Please click "Import Demo Data" button only once and wait, it can take a couple of minutes.', 'jannah' ); ?></li>
			</ul>
		</div>

		<div class="ocdi__intro-text">
			<p><?php esc_html_e( 'Before you begin, make sure all the required plugins are activated.', 'jannah' ); ?></p>
		</div>
			...
			<?php foreach ( $this->import_files as $index => $import_file ) :	?>
				<div class="theme">
					<div class="theme-screenshot" data-value="<?php echo esc_attr( $index ); ?>">
						...
						<span class="more-details"><?php echo esc_html__( 'Import', 'jannah' ); ?></span>
						<?php
							if( ! empty( $import_file['new_demos'] ) ){
								echo '<span class="new-demo">'. esc_html__( 'New', 'jannah' ) .'</span>';
							}
						?>
					</div>
					...
					<?php
						if( ! empty( $import_file['import_demo'] )){ ?>
							<div class="theme-actions">
								<a class="tie-live-demo button button-secondry" target="_blank" href="<?php echo esc_url( $import_file['import_demo'] ); ?>"><?php echo esc_html__( 'Live Demo', 'jannah' ); ?></a>
							</div>
							<?php
						}
					?>
...
<?php // by TieLabs ***************************************************************?>
				...
				<div class="theme-wrap wp-clearfix">
					...
					<div class="theme-about wp-clearfix">
						...
						<div class="tie-message-hint">
							<h4>
								<?php esc_html_e( 'Important Notes:', 'jannah' ); ?>
							</h4>

							<ol>
								<li><?php esc_html_e( 'We recommend to run Demo Import on a clean WordPress installation.', 'jannah' ); ?></li>
								<li><?php esc_html_e( 'The Demo Import will not import the images we have used in our live demos, due to copyright / license reasons.', 'jannah' ); ?></li>
								<li><?php esc_html_e( 'No existing posts, pages, categories, images, custom post types or any other data will be deleted or modified.', 'jannah' ); ?></li>
								<li><?php esc_html_e( 'Posts, pages, images, widgets and menus will get imported.', 'jannah' ); ?></li>
								<li><?php esc_html_e( 'Before you begin, make sure all the required plugins are activated.', 'jannah' ); ?></li>
								<li><?php esc_html_e( 'Do not run the Demo Import multiple times one after another, it will result in double content.', 'jannah' ); ?></li>
							</ol>
						</div>
					...
					<div class="theme-actions">
						<div class="import-button">
							<a id="tie-install-demo" class="tie-primary-button button button-primary button-hero" href="#"><?php echo esc_html__( 'Import', 'jannah' ); ?></a>
							<a id="tie-view-demo" class="tie-primary-button button button-hero" target="_blank" href="#"><?php echo esc_html__( 'Live Demo', 'jannah' ); ?></a>
						</div>

						<div class="imported-buttons">
							<a class="tie-primary-button button button-primary button-hero" href="<?php echo admin_url( 'admin.php?page=tie-theme-options' ) ?>"><?php echo esc_html__( 'Theme Options', 'jannah' ); ?></a>
							<a class="tie-primary-button button button-hero" target="_blank" href="<?php echo esc_url(home_url( '/' )) ?>"><?php echo esc_html__( 'Visit Site', 'jannah' ); ?></a>
						</div>
					</div>
					...
					<div class="ocdi__response js-tie-ajax-response js-ocdi-ajax-response">
						...
						<div class="tie-loading-wrapper">
							<h3><?php esc_html_e( 'Importing Demo Content...', 'jannah' ); ?></h3>
							<h4><?php esc_html_e( 'Please be patient and do not navigate away from this page while the import is in progress.	This can take a while if your server is slow.', 'jannah' ); ?></h4>
							<h4><?php esc_html_e( 'You will be notified via this page when the import is completed.', 'jannah' ); ?></h4>
						</div>
					</div>
		...
		if ( $this->plugin_page === $hook ){
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'texts'        => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'jannah' ),
					),
				)
			);
		}
	...
	public function import_demo_data_ajax_callback(){
		...
		if ( ! $use_existing_importer_data ){
			...
			if ( ! empty( $_FILES ) ){ // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'jannah' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ){ // Use predefined import files from wp filter: pt-ocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ){
					...
					OCDI_Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'jannah' )
					);
				}
				...
				$log_added = OCDI_Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'jannah' ),
				...
				if( ! empty( $this->import_files[ $this->selected_index ]['import_file_name'] )){
					update_option( 'tie_jannah_installed_demo', $this->import_files[ $this->selected_index ]['import_file_name']);
				}
			}
			else {

				// Send JSON Error response to the AJAX call.
				wp_send_json( esc_html__( 'No import files specified!', 'jannah' ) );
			}
		}
	...
	//TieLabs *************
		...
		if ( ! empty( $this->selected_import_files['settings'] ) && empty( $this->frontend_error_messages ) ){
			...
			do_action( 'tie_demo_import_settings', $this->selected_import_files['settings'] );
		...
		$log_added = OCDI_Helpers::append_to_file(
			sprintf(
				__( 'The Demo has been imported in % seconds.', 'jannah' ),
				$completed
			),
			$this->log_file_path,
			esc_html__( 'Completed', 'jannah' )
		);
	//END *************
		...
		if ( empty( $this->frontend_error_messages ) ){
			...
			$response['message'] = '<h3 class="is-done">' .__( 'That\'s it, all done!', 'jannah' ). '</h3>'.
				'<h4>'.
					sprintf(
						__( 'The demo import has finished. Now you can see the result at %1$syour site%2$s or start customize via %3$sTheme Options%2$s.', 'jannah' ),
			...
			$response['message'] = sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo import has finished. Please check your page and make sure that everything has imported correctly. If it did, you can deactivate the %3$sOne Click Demo Import%4$s plugin, because it has done its job.%5$s', 'jannah' ),
			...
			*/
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', 'jannah' ),
	...
	private function import_content( $import_file_path ){
		...
		if ( ! empty( $import_file_path ) ){
			...
			$log_added = OCDI_Helpers::append_to_file(
				...
				$this->log_file_path,
				esc_html__( 'Importing content', 'jannah' )
			);
		}
	...
	private function import_widgets( $widget_import_file_path ){
		...
		if ( is_wp_error( $results ) ){
			...
			OCDI_Helpers::log_error_and_send_ajax_response(
				...
				$this->log_file_path,
				esc_html__( 'Importing widgets', 'jannah' )
			);
		}
	...
	private function import_customizer( $customizer_import_file_path ){
		...
		if ( is_wp_error( $results ) ){
			...
			OCDI_Helpers::log_error_and_send_ajax_response(
				$results->get_error_message(),
				$this->log_file_path,
				esc_html__( 'Importing customizer settings', 'jannah' )
			);
		}

		// Add this message to log file.
		$log_added = OCDI_Helpers::append_to_file(
			esc_html__( 'Customizer settings import finished!', 'jannah' ),
			$this->log_file_path,
			esc_html__( 'Importing customizer settings', 'jannah' )
		);
	}
	...
	public function new_ajax_request_maybe( $data ){
		...
		if ( $time > apply_filters( 'pt-ocdi/time_for_one_ajax_call', 10 ) ){
			...
			$log_added = OCDI_Helpers::append_to_file(
				__( 'Completed AJAX call number: ', 'jannah' ) . $this->ajax_call_number . PHP_EOL . $message,
				$this->log_file_path,
				''
			);
