...
/*-----------------------------------------------------------------------------------*/
add_action( 'add_meta_boxes', 'jannah_add_custom_meta_boxes' );
function jannah_add_custom_meta_boxes(){
	...
	add_meta_box(
		...
		JANNAH_THEME_NAME .' - '. esc_html__( 'Front Page', 'jannah' ),
		'jannah_custom_meta_frontpage',
		'page',
		'side',
		'low'
	);
...
# Secondry post title
/*-----------------------------------------------------------------------------------*/
add_action( 'edit_form_after_title', 'jannah_secondry_post_title', 40 );
...
/*-----------------------------------------------------------------------------------*/
function jannah_custom_meta_frontpage(){
	...
	if( get_option( 'show_on_front' ) == 'page' ){
		...
		if( $current_page_id == $front_page_id ){
			...
			$notice = '
				<p>'. sprintf( esc_html__( 'Current Front Page: %s', 'jannah' ), $page ) .'</p>
			';
		}
	}
	...
	$option = array(
		'name'   => esc_html__( 'Set as the site Front Page?', 'jannah' ),
		'id'     => 'tie-set-front-page',
		'type'   => 'checkbox',
	);

	jannah_build_option( $option, 'page_on_front', $data  );
...
function jannah_custom_meta_boxes(){
	...
	if( get_post_type() == 'post' ){
		...
		$settings_tabs['source-via'] = array(
			'icon'  => 'share-alt2',
			'title' => esc_html__( 'Source and Via', 'jannah' ),
		);
	}
