...
if( ! function_exists( 'jannah_lazyload_image_attributes' )){
	...
	function jannah_lazyload_image_attributes( $attr, $attachment, $size ) {
		...
		if( JANNAH_JETPACK_IS_ACTIVE && in_array( 'photon', Jetpack::get_active_modules() ) && in_array( 'the_content', $GLOBALS['wp_current_filter'] ) ){
...
# Gravatar Lazyload
/*-----------------------------------------------------------------------------------*/
if( ! function_exists( 'jannah_lazyload_avatar' )){

	add_filter( 'get_avatar', 'jannah_lazyload_avatar', 10, 6 );
	function jannah_lazyload_avatar( $avatar, $user, $size, $default, $alt, $args ){

		if( jannah_get_option( 'lazy_load' ) && ! is_admin() && ! is_feed() && ! in_array( 'the_content', $GLOBALS['wp_current_filter'] ) ){
			...
			$avatar = sprintf(
				...
				esc_attr( $args['alt'] ),
				JANNAH_TEMPLATE_URL.'/images/tie-empty-small.png',
				esc_url( $args['url'] ),
				esc_attr( join( ' ', $class ) ),
				(int) $args['height'],
				(int) $args['width'],
