 ...
 */
 ...
 if( ! class_exists( 'JANNAH_BUILDER_WIDGETS' )){

 	class JANNAH_BUILDER_WIDGETS{
		...
		function _print_scripts(){
	 		...
	 		if( ! empty( $screen->base ) && $screen->base == 'post' && $screen->id == 'page' ){
	 			...
	 			wp_enqueue_script(
          'tie-builder-widgets',
          JANNAH_TEMPLATE_URL .'/framework/admin/assets/page-widgets.js',
          array(
            'jquery',
            'jquery-ui-sortable',
            'jquery-ui-draggable',
		...
		function _ajax_save_widget(){
			...
			$error        = '<p>' . esc_html__( 'An error has occured. Please reload the page and try again.', 'jannah' ) . '</p>';
      ...
      echo "
        <div id=\"tie-show-sections-sidebars-wrap\">
          <h2>". esc_html__( 'TieLabs Page Builder widgets areas', 'jannah' )."</h2>
          <div>
     ...
     * We need a better Idea :)
 	...
 	new JANNAH_BUILDER_WIDGETS();
 }




function jannah_builder_widgets( $sections = array() ) {
	...
	<div id="tie-sidebars-customize" class="tie-popup-block tie-popup-window">
		...
		<div class="tie-builder-item-top-container">
			<h2><?php esc_html_e( 'Manage Widgets', 'jannah' ) ?></h2>
    			...
    			<div id="widgets-left">
    				<div id="available-widgets" class="widgets-holder-wrap">
    					<div class="sidebar-name">
    						...
    						<h3><?php esc_html__('Available Widgets', 'jannah'); ?> <span id="removing-widget"><?php esc_html__('Deactivate', 'jannah'); ?> <span></span></span></h3>
    					</div>
    					<div class="widget-holder">
    						<p class="description"><?php esc_html__('Drag widgets from here to a sidebar on the right to activate them. Drag widgets back here to deactivate them and delete their settings.', 'jannah'); ?></p>
    						<div id="widget-list">
    							<?php wp_list_widgets(); ?>
    						</div>
    						<br class='clear' />
    		...
    		<div class="widget-liquid-right">
              ...
              if( ! empty( $sections ) && is_array( $sections ) ){
                  ...
                  echo '<div id="wrap-'. $section_id .'" class="widgets-holder-wrap">';
                    wp_list_widget_controls( $section_id, esc_html__('Section Widgets Area', 'jannah' ) );
                  echo '</div>';
      					}
              }
    				?>
	...
	<div class="widgets-chooser">
		...
		<div class="widgets-chooser-actions">
			<button class="button-secondary"><?php esc_html_e( 'Cancel', 'jannah'  ); ?></button>
			<button class="button-primary"><?php esc_html_e( 'Add Widget', 'jannah'  ); ?></button>
		</div>
	</div>
	<?php
}
