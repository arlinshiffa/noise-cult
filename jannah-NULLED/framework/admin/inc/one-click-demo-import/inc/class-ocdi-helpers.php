...
class OCDI_Helpers {
	...
	public static function download_import_files( $import_file_info, $start_date = '' ){
		...
		$demo_local_path = get_template_directory() . '/demo/' . $import_file_info['import_file_name'] .'/';
		$demo_import_content = file_get_contents( $demo_local_path . 'demo-content-import-file.xml' );
		...
		if ( ! empty( $import_file_info['import_widget_file_url'] ) ){
			...
			$demo_import_widgets_content = file_get_contents( $demo_local_path . 'demo-widgets-import-file.json' );
...
//TieLabs ****************
		...
		if ( ! empty( $import_file_info['import_settings_file_url'] ) ){
			...
			$demo_import_settings_content = file_get_contents( $demo_local_path . 'demo-settings-import-file.json' );
		...
		if ( ! empty( $import_file_info['import_woocommerce_file_url'] ) && JANNAH_WOOCOMMERCE_IS_ACTIVE ){
...
//END ***********************
		...
		if ( ! empty( $import_file_info['import_customizer_file_url'] ) ){
			...
			$demo_import_customizer_content = file_get_contents( $demo_local_path . 'demo-customizer-import-file.dat' );
	...
	private static function get_content_from_url( $url, $file_name = 'Import file' ){
		...
		if ( empty( $url ) ){
			return new WP_Error(
				'url_not_defined',
				sprintf(
					__( 'URL for %s%s%s file is not defined!', 'jannah' ),
					'<strong>',
					$file_name,
					'</strong>'
				)
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ){
			...
			return new WP_Error(
				'file_fetching_error',
				sprintf(
					__( 'An error occurred while fetching %s%s%s file from the server!%sReason: %s - %s.', 'jannah' ),
					'<strong>',
					$file_name,
					'</strong>',
					'<br>',
	...
	public static function write_to_file( $content, $file_path ){
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ){
			return new WP_Error(
				...
				sprintf(
					__( 'An error occurred while writing file to your server! Tried to write a file to: %s%s.', 'jannah' ),
					'<br>',
					$file_path
				)
			);
	...
	public static function data_from_file( $file_path ){
		...
		if ( ! $data ){
			return new WP_Error(
				...
				sprintf(
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %s%s.', 'jannah' ),
					'<br>',
					$file_path
				)
			);
	...
	private static function check_wp_filesystem_credentials(){
		...
		if ( ! ( 'direct' === get_filesystem_method() ) ){
			return new WP_Error(
				...
				sprintf(
					__( 'This WordPress page does not have %sdirect%s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %s.', 'jannah' ),
					'<strong>',
					'</strong>',
		...
		$plugin_page_setup = apply_filters( 'pt-ocdi/plugin_page_setup', array(
				...
				'page_title'  => esc_html__( 'One Click Demo Import', 'jannah' ),
				'menu_title'  => esc_html__( 'Import Demo Data', 'jannah' ),
				'capability'  => 'import',
		...
		if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ){
			return new WP_error(
				...
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'jannah' )
			);
		}
		...
		if ( ! WP_Filesystem( $creds ) ){
			return new WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'jannah' )
			);
		}
	...
	public static function verify_ajax_call(){
		...
		if ( ! current_user_can( 'import' ) ){
			wp_die(
				sprintf(
					__( '%sYour user role isn\'t high enough. You don\'t have permission to import demo data.%s', 'jannah' ),
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ){
		...
		if ( empty( $content_file_info['file'] ) || isset( $content_file_info['error'] ) ){
			...
			self::log_error_and_send_ajax_response(
				__( 'Please upload XML file for content import. If you want to import widgets or customizer settings only, please use Widget Importer & Exporter or the Customizer Export/Import plugin.', 'jannah' ),
				$log_file_path,
				esc_html__( 'Upload files', 'jannah' )
			);
		}
		...
		if ( $widget_file_info && ! isset( $widget_file_info['error'] ) ){
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Widget file was not uploaded. Error: %s', 'jannah' ),
		...
		if ( $customizer_file_info && ! isset( $customizer_file_info['error'] ) ){
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Customizer file was not uploaded. Error: %s', 'jannah' ),
		...
		$log_added = self::append_to_file(
			__( 'The import files were successfully uploaded!', 'jannah' ) . self::import_file_info( $selected_import_files ),
			$log_file_path,
			esc_html__( 'Upload files', 'jannah' )
		);
	...
	public static function import_file_info( $selected_import_files ){
		return PHP_EOL .
		sprintf(
			__( 'MAX EXECUTION TIME = %s', 'jannah' ),
			ini_get( 'max_execution_time' )
		) . PHP_EOL .
		sprintf(
			__( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sWidget file = %4$s%1$sCustomizer file = %5$s', 'jannah' ),
			PHP_EOL,
			get_site_url(),
			$selected_import_files['content'],
			empty( $selected_import_files['widgets'] ) ? esc_html__( 'not defined!', 'jannah' ) : $selected_import_files['widgets'],
			empty( $selected_import_files['customizer'] ) ? esc_html__( 'not defined!', 'jannah' ) : $selected_import_files['customizer']
		);
	}
