...
function jannah_build_option( $value, $option_name, $data ){
	...
	switch ( $type ){
		...
		//number Option ----------
		case 'number':
			echo"<input style=\"width:60px\" min=\"-1000\" max=\"1000\" $item_id_attr $name_attr type=\"number\" value=\"". esc_attr($current_value) ."\" $placeholder_attr>";
		break;
		...
		case 'visual':
			...
			echo "<ul id=\"tie_$item_id\" class=\"tie-options $columns\">";
				...
				$images_path = empty( $external_images ) ? JANNAH_TEMPLATE_URL .'/framework/admin/assets/images/' : '';
