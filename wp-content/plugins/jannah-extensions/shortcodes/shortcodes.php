...
function jannah_extensions_shortcodes_enqueue_scripts() {
	...
	if( true === $load_css_js ) {
		wp_enqueue_style ( 'jannah-extensions-shortcodes-styles',  plugins_url( 'shortcodes/assets/style.css', __FILE__ ),     array(), '', 'all' );
		wp_enqueue_script( 'jannah-extensions-shortcodes-scripts', plugins_url( 'shortcodes/assets/js/scripts.js', __FILE__ ), array( 'jquery' ), false, true );
	}
}
...
function jannah_extensions_sc_post_slideshow( $atts, $content = null ) {
	...
	do_action( 'jannah_extensions_sc_before_post_slideshow' );
